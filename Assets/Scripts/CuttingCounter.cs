...
public class CuttingCounter : BaseCounter
{
    ...
    public override void InteractAlternate(Player player)
    {
        if (this.HasKitchenObject() && HasCuttingRecipeSO(GetKitchenObject()))
        {
            ...
            OnProgressChanged?.Invoke(this, new OnProgressChangedEventArgs { progressNormalized = (float)cutTimes/cutRequired });
    ...
    private KitchenObjectSO GetCutted(KitchenObject input)
    {
        ...
        Debug.LogWarning("No CuttingRecipieSO found for" + input.name + "in CuttingCounter" + this.name);
        return null;
    }
